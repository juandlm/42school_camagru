...
class Application
{
    ...
    public function __construct() {
        ...
        if (!$this->_url_controller) {
            $page = new \Camagru\Controller\HomeController();
            $page->index();
        } elseif (file_exists(APP . 'Controller/' . ucfirst($this->_url_controller) . 'Controller.php')) {
            $controller = "\\Camagru\\Controller\\" . ucfirst($this->_url_controller) . 'Controller';
            ...
            if (method_exists($this->_url_controller, $this->_url_action) &&
                ...
                if (strlen($this->_url_action) == 0) {
                    ...
                    $page = new \Camagru\Controller\ErrorController();
                    $page->index();
                }
            }
        } else {
