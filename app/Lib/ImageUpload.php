...
class ImageUpload
{
	...
	public function ajaxUpload($usr_id, $upl_type, $custom = false) {
		try {
			...
			$this->img_path = "public/img/userupload/" . $fileName;
			$destination = ROOT . $this->img_path;
			$moveFile = $custom === true ? rename($image["tmp_name"], $destination)
						: move_uploaded_file($image["tmp_name"], $destination);
			if ($moveFile) {
				$this->img_url = URL . $this->img_path;
				$imageManager = new \Camagru\Model\ImageManager();
				if ($imageManager->newUpload($usr_id, $this->img_path, $upl_type)) {
					return (json_encode(
						["status" => true,
						"url" => $this->img_url]
					));
